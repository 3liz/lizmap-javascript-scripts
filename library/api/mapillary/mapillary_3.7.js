...
lizMap.events.on({
    uicreated: () => {
        ...
        // Load CSS
        $("head").append('<link href="https://unpkg.com/mapillary-js@4.1.2/dist/mapillary.css" rel="stylesheet"/>');
        $("head").append(`
        <style type="text/css">
    ...
    minidockopened: e => {
        if (e.id === 'Mapillary') {
            // Put new OL map on top when Lizmap < 3.8
            // (newOlMap getter only exists in Lizmap >=3.8)
            if (lizMap.mainLizmap.newOlMap === undefined) {
            ...
            const token = 'YOUR_MAPILLARY_TOKEN_HERE';

            const mapillaryVTSource = new lizMap.ol.source.VectorTile({
            ...
            const mapillaryVT = new lizMap.ol.layer.VectorTile({
                title: 'mapillary-vt',
            ...
            lizMap.mainLizmap.map.on('loadend', evt => {
                for (const layer of evt.map.getLayers().getArray()) {
                    if (layer.get('title') === 'mapillary-vt') {
                        let featuresInCurrentExtent;
                        if (layer.getFeaturesInExtent) { // >= OL10
                            featuresInCurrentExtent = layer.getFeaturesInExtent(evt.map.getView().calculateExtent());
                        } else if (layer.getSource().getFeaturesInExtent) { // < OL10
                            featuresInCurrentExtent = layer.getSource().getFeaturesInExtent(evt.map.getView().calculateExtent());
                        }
            ...
            // Draw layer
            const drawSource = new lizMap.ol.source.Vector();
            const drawLayer = new lizMap.ol.layer.Vector({
                title: 'mapillary-pov',
                source: drawSource,
            ...
            viewer.on('position', async () => {
                ...
                const coords = lizMap.ol.proj.transform([position.lng, position.lat], 'EPSG:4326', lizMap.mainLizmap.projection);
                ...
                if (!lizMap.ol.extent.containsCoordinate(smallerExtent, coords)) {
            ...
            lizMap.mainLizmap.map.on('singleclick', evt => {
                ...
                if (imageId) {
                    ...
                    drawSource.addFeature(new lizMap.ol.Feature({
                        geometry: new lizMap.ol.geom.Point(features[0].getFlatCoordinates())
                    }));
    ...
    minidockclosed: e => {
        if (e.id === 'Mapillary') {
            // Put new OL map at bottom when Lizmap < 3.8
            // (newOlMap getter only exists in Lizmap >=3.8)
            if (lizMap.mainLizmap.newOlMap === undefined) {
                lizMap.mainLizmap.newOlMap = true;
            }
